...
parsePath = function(path, trianglify){
  var ref$, shape, hole, svg, ds, t1, paths, shapes, intersect, i$, to$, i, j$, to1$, j, gs, len$, g, base, len1$, idx, p, ret, pts, faces;
  ...
  paths = path.replace(/Z/g, 'z').split('z').filter(function(it){
    ...
    pts = [];
    for (i$ = 0, to$ = len + 1, step$ = len ? len / 50 : 1; step$ < 0 ? i$ >= to$ : i$ <= to$; i$ += step$) {
      i = i$;
